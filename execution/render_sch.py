...
def render_schematic(sch_path, output_image):
        ...
        try:
            ...
            if at_match:
                ...
                if "R" in lib_id:
                    ...
                    w, h = 4, 8
                elif "NE555" in lib_id:
                    w, h = 12, 14
                elif "LED" in lib_id or "D" in lib_id:
                    w, h = 4, 8

                # Rectángulo amarillo claro con borde azul (estilo KiCad clásico)
                rect = Rectangle((x - w/2, y - h/2), w, h, linewidth=1,
                                 edgecolor='#800000', facecolor='#FFFFCC', alpha=0.8)
                ax.add_patch(rect)
    ...
    for match in re.finditer(label_pattern, content):
        ...
        x, y = float(x), float(y)
        ax.text(x, y, text, ha='left', va='center', fontsize=9, color='#006600', weight='bold',
