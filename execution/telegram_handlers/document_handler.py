from telegram_handlers.pdf_handler import handle_pdf_from_document
...
def handle_document(msg, sender_id, run_tool):
    ...
    run_tool("telegram_tool.py", ["--action", "send", "--message",
             f"📂 Recibí `{file_name}`. Leyendo contenido...", "--chat-id", sender_id])
    ...
    if file_name.lower().endswith(".ses"):
        run_tool("telegram_tool.py", ["--action", "send", "--message",
                 "🛰️ Archivo de sesión de DeepPCB detectado. Fusionando pistas...", "--chat-id", sender_id])
...
'''
        ...
        if "MERGE_OK" in res_exec.get("stdout", ""):
            ...
            run_tool("telegram_tool.py", ["--action", "send-document", "--file-path", pcb_updated, "--chat-id", sender_id,
                     "--caption", "✅ ¡Enrutado completado!\\nHe integrado las pistas de DeepPCB en tu diseño. Ya puedes usar `/fabricar`."])
    ...
    # --- Lógica de Documentos PDF (delegada al handler especializado) ---
    if file_name.lower().endswith(".pdf"):
        return handle_pdf_from_document(msg, sender_id, run_tool)
