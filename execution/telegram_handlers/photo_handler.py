def handle_photo(msg, sender_id, run_tool):
    ...
    if not caption.strip():
        caption = "Describe qué ves en esta imagen."
    ...
    if "/diseñar" in caption_lower or "/disenar" in caption_lower or "/design" in caption_lower:
        ...
        if not design_prompt:
            ...
            run_tool("telegram_tool.py", ["--action", "send", "--message",
                     "⚠️ No incluiste una descripción, así que intentaré deducir la función del circuito.", "--chat-id", sender_id])

        run_tool("telegram_tool.py", ["--action", "send", "--message",
                 f"🤖 ¡Entendido! Analizando tu dibujo para: '{design_prompt}'.\nDame un momento para pensar como ingeniero...", "--chat-id", sender_id])
                ...
                try:
                    ...
                    if i == 0:
                        run_tool("telegram_tool.py", [
                                 "--action", "send", "--message", "🤔 El formato de mi respuesta inicial no era correcto. Intentando corregirlo...", "--chat-id", sender_id])
        ...
        run_tool("telegram_tool.py", ["--action", "send", "--message",
                 "🏭 Generando paquete completo de fabricación (Gerber + Drill)...", "--chat-id", sender_id])

        try:
            with open("execution/img_to_gerber.py", "r") as f:
                script_gerber = f.read()
            ...
            if res_gerber.get("status") == "success" and res_drill.get("status") == "success":
                ...
                with open("execution/create_manufacturing_zip.py", "r") as f:
                    script_zip = f.read()
                ...
                if res_zip.get("status") == "success":
                    ...
                    if os.path.exists(generated_zip):
                        run_tool("telegram_tool.py", ["--action", "send-document", "--file-path",
                                 generated_zip, "--chat-id", sender_id, "--caption", "📦 Manufacturing Pack (ZIP)"])
                else:
        ...
        run_tool("telegram_tool.py", ["--action", "send", "--message",
                 "🏭 Generando archivo Gerber (Top Copper)...", "--chat-id", sender_id])
        ...
        try:
            ...
            if res_sandbox and res_sandbox.get("status") == "success":
                ...
                if os.path.exists(generated_file):
                    run_tool("telegram_tool.py", ["--action", "send-document", "--file-path",
                             generated_file, "--chat-id", sender_id, "--caption", "Gerber Top Copper"])
            else:
        ...
        run_tool("telegram_tool.py", ["--action", "send", "--message",
                 "🔩 Detectando agujeros y generando Excellon (.drl)...", "--chat-id", sender_id])
        ...
        try:
            ...
            if res_sandbox and res_sandbox.get("status") == "success":
                ...
                if os.path.exists(generated_file):
                    run_tool("telegram_tool.py", ["--action", "send-document", "--file-path",
                             generated_file, "--chat-id", sender_id, "--caption", "Excellon Drill File"])
            else:
        ...
        run_tool("telegram_tool.py", ["--action", "send", "--message",
                 "⚙️ Convirtiendo imagen a G-Code...", "--chat-id", sender_id])
        try:
            ...
            if res_sandbox and res_sandbox.get("status") == "success":
                ...
                if os.path.exists(generated_file):
                    run_tool("telegram_tool.py", ["--action", "send-document", "--file-path",
                             generated_file, "--chat-id", sender_id, "--caption", "G-Code generado desde imagen"])
            else:
                err_msg = res_sandbox.get('stderr') or "Error desconocido"
        ...
        run_tool("telegram_tool.py", ["--action", "send", "--message",
                 "👀 Analizando imagen...", "--chat-id", sender_id])
