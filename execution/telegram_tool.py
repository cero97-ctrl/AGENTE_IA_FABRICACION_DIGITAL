            ...
            if update_id >= offset:
                ...
                if ALLOWED_USERS != "*":
                    ...
                    if msg_chat_id not in allowed_list:
                        print(
                            f"⚠️ Ignorando mensaje de {msg_chat_id} (No autorizado. Permitidos: '{ALLOWED_USERS}')", file=sys.stderr)
                        continue
...
def get_chat_id():
    ...
    for _ in range(5):
        try:
            ...
            if results:
                ...
                print(json.dumps({"status": "success", "users": user_list,
                      "message": "Copia el ID del estudiante que necesites."}))
                return
    ...
    print(json.dumps(
        {"status": "error", "message": "No se encontraron mensajes recientes. Pide al estudiante que envíe 'Hola' a tu bot ANTES de ejecutar esto."}))
    sys.exit(1)
