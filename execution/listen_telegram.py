...
# DEBE estar antes de cualquier import local
sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
# También asegurar que execution/ esté en el path
...
def main():
    ...
    offset_path = os.path.join(os.path.dirname(os.path.dirname(
        os.path.abspath(__file__))), ".tmp", "telegram_offset.txt")
        ...
        while True:
            ...
            if response and response.get("status") == "success":
                ...
                for msg in messages:
                    ...
                    reply_text, is_voice_interaction, voice_lang_short, final_msg = handle_message(
                        content, sender_id, run_tool)

                    # 3. Enviar respuesta a Telegram
                    if reply_text:
                        print(f"   📤 Enviando respuesta: '{reply_text[:60]}...'")
                        res = run_tool("telegram_tool.py", ["--action", "send",
                                       "--message", reply_text, "--chat-id", sender_id])
                        ...
                        if is_voice_interaction and reply_text:
                            ...
                            # Generar audio
                            tts_res = run_tool("text_to_speech.py", [
                                               "--text", reply_text[:500], "--output", audio_path, "--lang", voice_lang_short])
                            if tts_res and tts_res.get("status") == "success":
                                run_tool("telegram_tool.py", ["--action", "send-voice",
                                         "--file-path", audio_path, "--chat-id", sender_id])
            ...
            if time.time() - last_health_check > HEALTH_CHECK_INTERVAL:
                ...
                if admin_id:
                    ...
                    if res and res.get("alerts"):
                        ...
                        run_tool("telegram_tool.py", ["--action", "send",
                                 "--message", alert_msg, "--chat-id", admin_id])
